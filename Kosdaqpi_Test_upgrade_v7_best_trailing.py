...
# -------------------- 코스피(122630/252670) 튜닝 파라미터 --------------------
KOSPI_252670_DISPARITY11_TH = 106
KOSPI_122630_DISPARITY_LOW = 98
KOSPI_122630_DISPARITY_HIGH = 105
KOSPI_MIN_HOLD_DAYS = 0  # 0이면 비활성(원본 동일)

# 코스피 매수 파라미터(최종 고정: buy_wide_disp)
...
# -------------------- 봇 기준 절반 트레일링 스탑 백테스트 --------------------
ENABLE_BACKTEST_TRAILING_STOP = True
TRAILING_STOP_SPLIT_RATE = 0.5          # 보유 물량 절반
TRAILING_STOP_CANDLE_COUNT = 20
TRAILING_STOP_MULTIPLIER = 2.0
TRAILING_STOP_MIN_RATE = 2.0
# -----------------------------------------------------------------------------
...
def get_active_position_count(now_invest_list):
    return len({data['stock_code'] for data in now_invest_list if data.get('InvestMoney', 0) > 0})


def calc_trailing_callback_rate(stock_df, buy_price):
    if ENABLE_BACKTEST_TRAILING_STOP is False:
        return 0.0
    try:
        hl_avg = float(stock_df['hl_range_avg'].values[0])
        base_price = float(buy_price)
        if hl_avg <= 0 or base_price <= 0:
            return 0.0
        callback_rate = (hl_avg * TRAILING_STOP_MULTIPLIER / base_price) * 100.0
        return max(callback_rate, TRAILING_STOP_MIN_RATE)
    except Exception:
        return 0.0


def build_invest_data_list(stock_code, real_invest_money, buy_price, buy_date, stock_df):
    result = []

    trailing_money = 0.0
    if ENABLE_BACKTEST_TRAILING_STOP:
        trailing_money = real_invest_money * TRAILING_STOP_SPLIT_RATE
    normal_money = real_invest_money - trailing_money

    if normal_money > 0:
        result.append({
            'stock_code': stock_code,
            'InvestMoney': normal_money,
            'FirstMoney': normal_money,
            'BuyPrice': buy_price,
            'DolPaCheck': False,
            'Date': str(buy_date),
            'UseTrailingStop': False,
            'TrailingCallbackRate': 0.0,
            'TrailingHighPrice': buy_price,
        })

    if trailing_money > 0:
        result.append({
            'stock_code': stock_code,
            'InvestMoney': trailing_money,
            'FirstMoney': trailing_money,
            'BuyPrice': buy_price,
            'DolPaCheck': False,
            'Date': str(buy_date),
            'UseTrailingStop': True,
            'TrailingCallbackRate': calc_trailing_callback_rate(stock_df, buy_price),
            'TrailingHighPrice': buy_price,
        })
...
for date in combined_df.index.unique():
            ...
            if len(stock_data) == 1:
                ...
                if stock_code in ["233740","251340"]:
                    ...
                    if stock_code in CLOSE_BASED_CUT_CODES:
                        if CutPrice >= stock_data['close'].values[0]:
                            IsSellGo = True
                            SellPriceCandidates.append(CutPrice)
                    else:
                    ...
                    if ENABLE_233740_HARD_STOP and stock_code == "233740":
                        ...
                        if stock_data['low'].values[0] <= HardStopPrice:
                            ...
                            if NowOpenPrice <= HardStopPrice:
                                SellPriceCandidates.append(NowOpenPrice)
                            else:
                                SellPriceCandidates.append(HardStopPrice)

                    if investData.get('UseTrailingStop') == True and investData.get('TrailingCallbackRate', 0) > 0:
                        prev_trailing_high = float(investData.get('TrailingHighPrice', investData['BuyPrice']))
                        trailing_stop_price = prev_trailing_high * (1.0 - investData['TrailingCallbackRate'] / 100.0)
                        if stock_data['low'].values[0] <= trailing_stop_price:
                            IsSellGo = True
                            if NowOpenPrice <= trailing_stop_price:
                                SellPriceCandidates.append(NowOpenPrice)
                            else:
                                SellPriceCandidates.append(trailing_stop_price)
                        investData['TrailingHighPrice'] = max(prev_trailing_high, float(stock_data['high'].values[0]))

                    if len(SellPriceCandidates) > 0:
                        SellPrice = min(SellPriceCandidates)
                ...
                #조건 만족시 매도 한다!
                    ...
                    if stock_code == "252670":
                        ...
                        if stock_data['Disparity11'].values[0] > KOSPI_252670_DISPARITY11_TH:
                            ...
                            if  PrevClosePrice < stock_data['ma3_before'].values[0]: 
                                IsSellGo = True
                                SellPriceCandidates.append(SellPrice)
                    ...
                    # KODEX 레버리지
                        ...
                        if (stock_data['prevLow2'].values[0] < stock_data['prevLow'].values[0] or stock_data['prevVolume'].values[0] < total_volume) and (Disparity < KOSPI_122630_DISPARITY_LOW or Disparity > KOSPI_122630_DISPARITY_HIGH):
                            print("hold..")
                        else:
                            IsSellGo = True
                            SellPriceCandidates.append(SellPrice)
    ...
    if get_active_position_count(NowInvestList) < int(DivNum)/2 and int(date_object.strftime("%Y")) >= StartYear:
                ...
                if stock_code in ["122630","252670"]:
                    ...
                    if IsBuyGo == True :
                        ...
                        if IsNoWay == True:
                            ...
                            if get_active_position_count(NowInvestList) + Kosdaq_sell_cnt == 0:
                        ...
                        if Rate > 0:
                            ...
                            if BuyAmt > 0:
                                ...
                                for InvestData in build_invest_data_list(stock_code, RealInvestMoney, DolPaPrice, date, stock_data):
    ...
    #print("\n\n------------------------------------")
    for iData in NowInvestList:
        if iData['stock_code'] not in shown_codes and iData['InvestMoney'] > 0:
            shown_codes.add(iData['stock_code'])
   ...
   # print("------------------------------------\n\n")
    ...
    print("\n\n>>>>>>>>>>>>", InvestCoinListStr, "---> 투자개수 : ", get_active_position_count(NowInvestList))
    pprint.pprint(NowInvestList)
