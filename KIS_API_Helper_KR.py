...
def GetTopVolumeStockList():
    ...
    params = {
        ...
        "FID_TRGT_EXLS_CLS_CODE":"0111111101",
        "FID_INPUT_PRICE_1":"",
        "FID_INPUT_PRICE_2":"",
        "FID_VOL_CNT":"",
        "FID_INPUT_DATE_1":""
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        ...
        for stock in TopList:
            TopData = dict()
            TopData['Rank'] = stock['data_rank']
            TopData['StockCode'] = stock['mksc_shrn_iscd']
            TopData['StockName'] = stock['hts_kor_isnm']
            TopData['AvgVolume'] = stock['avrg_vol']
            TopData['AvgValue'] = stock['avrg_tr_pbmn']
            TopData['UpRate'] = stock['n_befr_clpr_vrss_prpr_rate']
            VolumeTopList.append(TopData)
            

        return VolumeTopList
    else:
...
def GetTopRateStockList():
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        ...
        for stock in TopList:
            TopData = dict()
            TopData['Rank'] = stock['data_rank']
            TopData['StockCode'] = stock['stck_shrn_iscd']
            TopData['StockName'] = stock['hts_kor_isnm']
            TopData['AvgVolume'] = stock['acml_vol']
            TopData['AvgValue'] = str(int(stock['acml_vol']) * int(stock['stck_prpr']))
            TopData['UpRate'] = stock['lwpr_vrss_prpr_rate']
            VolumeTopList.append(TopData)
    ...
    if IsOpen == True:
        ...
        if Is_CheckTody == True:
            ...
            try:
                ...
                if int(date_object.strftime("%d")) == day_n and df['volume'].iloc[-1] > 0:

                    Is_Ok = True
                    
                    #시가,종가,고가,저가가 똑같다? 장이 안열린 상황일 수 있다!
                    if df['open'].iloc[-1] == df['close'].iloc[-1] == df['high'].iloc[-1] == df['low'].iloc[-1]:
                        print("시가,종가,고가,저가가 똑같다! 장이 안열린 상황일 수 있다!")
                        
                        NotOpen = False
                        try:
                            df = Common.GetOhlcv1("KR", "005930",10) 
                            date = df.iloc[-1].name
                            ...
                            print("한번 더 체크!")
                            print(int(date_object.strftime("%d")) , " <<--- date_object")
                            print(day_n , " <<--- day_n")
