...
HARD_STOP_233740_PCT_TIGHT = 0.09  # 약세+고변동 구간
HARD_STOP_233740_PCT_BASE = 0.13    # 일반 구간
        ...
        if investData['InvestMoney'] > 0:
            ...
            if len(stock_data) == 1:
                ...
                if stock_code in ["233740","251340"]:
                    ...
                    # KODEX 코스닥150레버리지(233740) 컷레이트 완화: 0.25/0.35 -> 0.26/0.36
                    else:
            ...
            if stock_code not in today_sell_code and IsAlReadyInvest == False:
                ...
                if stock_code in ["122630","252670"]:
                    ...
                    # KODEX 레버리지
                        ...
                        if (stock_data['prevLow2'].values[0] < stock_data['prevLow'].values[0]) and (Disparity < KOSPI_122630_BUY_DISPARITY_LOW or Disparity > KOSPI_122630_BUY_DISPARITY_HIGH) and stock_data['prevRSI'].values[0] < KOSPI_122630_BUY_RSI_MAX and stock_data['prevOpen'].values[0] <= stock_data['prevClose'].values[0] :
                            IsBuyGo = True
