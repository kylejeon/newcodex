...
import numpy as np
import matplotlib
matplotlib.use("Agg")  # headless: plt.show() 차단 회피
import matplotlib.pyplot as plt
from datetime import datetime


# === Phase B BE_STOP 환경변수 ===
# BE_STOP_PCT: simple 모드 — max_close >= BuyPrice*(1+pct/100) 도달 후
#              today's low <= BuyPrice 이면 entry 청산 (ROI 0).
# BE_STOP_STEPPED='1': 가변 모드 — max_gain 단계별 stop_price 상승:
#   max_gain ≥ 15% → stop at entry × 1.10 (10% 수익 lock)
#   max_gain ≥ 10% → stop at entry × 1.05 (5% 수익 lock)
#   max_gain ≥  5% → stop at entry × 1.00 (entry break)
# BE_STOP_STEPPED 우선. 둘 다 0 이면 baseline.
BE_STOP_PCT = float(os.environ.get('BE_STOP_PCT', '0'))
BE_STOP_STEPPED = os.environ.get('BE_STOP_STEPPED', '0') == '1'
# 조용히 실행 (디버그 print 줄임). 기본 True.
QUIET_MODE = os.environ.get('PHASE_B_QUIET', '1') == '1'


def _be_stop_target_price(buy_price: float, max_gain_pct: float) -> float:
    """현재 max_gain 에 따른 stop 목표가. 트리거 안 함이면 0.0."""
    if BE_STOP_STEPPED:
        if max_gain_pct >= 15.0:
            return buy_price * 1.10
        if max_gain_pct >= 10.0:
            return buy_price * 1.05
        if max_gain_pct >= 5.0:
            return buy_price * 1.00
        return 0.0
    if BE_STOP_PCT > 0 and max_gain_pct >= BE_STOP_PCT:
        return buy_price
    return 0.0


_BE_STOP_ENABLED = BE_STOP_STEPPED or (BE_STOP_PCT > 0)

if QUIET_MODE:
    # print 를 무시 (결과 요약은 마지막에 직접 출력)
    import builtins as _builtins
    _original_print = _builtins.print
    _kept_keywords = ('--->>>', '<<<---', '최초 금액', '수익률:', 'MDD:', '연복리수익률', '------', '####', '====BE_STOP', 'BE_STOP 트리거', '승률:', '성공:', '실패:', '매매당 평균')

    def _quiet_print(*args, **kwargs):
        if not args:
            return
        s = " ".join(str(a) for a in args)
        if any(k in s for k in _kept_keywords):
            _original_print(*args, **kwargs)

    _builtins.print = _quiet_print

print(f"====BE_STOP CONFIG==== BE_STOP_PCT={BE_STOP_PCT}  STEPPED={BE_STOP_STEPPED}  QUIET={QUIET_MODE}")
        ...
        if investData['InvestMoney'] > 0:
            ...
            if len(stock_data) == 1:
                ...
                if stock_code in ["233740","251340"]:
                    ...
                    # === BE_STOP (KOSDAQ) — simple + stepped 지원 ===
                    BE_STOP_FIRED = False
                    if _BE_STOP_ENABLED and not IsSellGo:
                        _pc = stock_data['prevClose'].values[0]
                        if pd.notna(_pc) and _pc > 0:
                            investData['MaxCloseSinceBuy'] = max(
                                investData.get('MaxCloseSinceBuy', investData['BuyPrice']),
                                _pc,
                            )
                        _max_close = investData.get('MaxCloseSinceBuy', 0.0)
                        _max_gain_pct = (_max_close - investData['BuyPrice']) / investData['BuyPrice'] * 100.0
                        _stop_target = _be_stop_target_price(investData['BuyPrice'], _max_gain_pct)
                        if _stop_target > 0 and stock_data['low'].values[0] <= _stop_target:
                            IsSellGo = True
                            SellPrice = _stop_target
                            BE_STOP_FIRED = True
                            investData['BE_STOP_TRIGGERED'] = True
                            _stop_roi_pct = (_stop_target - investData['BuyPrice']) / investData['BuyPrice'] * 100.0
                            _original_print(f"BE_STOP 트리거 ({stock_code}) {str(date)[:10]} max_gain={_max_gain_pct:+.2f}% → stop sell @ {SellPrice:.0f} (lock {_stop_roi_pct:+.1f}%)")


                    #매일 매일 투자금 반영!
                    if BE_STOP_FIRED:
                        # stop target 청산 → ROI = (SellPrice - BuyPrice) / BuyPrice
                        _be_roi = (SellPrice - investData['BuyPrice']) / investData['BuyPrice']
                        investData['InvestMoney'] = investData['FirstMoney'] * (1.0 + _be_roi)
                    elif investData['DolPaCheck'] == False:
                ...
                #조건 만족시 매도 한다!
                    ...
                    # === BE_STOP (KOSPI) — simple + stepped 지원 ===
                    BE_STOP_FIRED_KP = False
                    if _BE_STOP_ENABLED and not IsSellGo:
                        _pc = stock_data['prevClose'].values[0]
                        if pd.notna(_pc) and _pc > 0:
                            investData['MaxCloseSinceBuy'] = max(
                    ...
                    if KOSPI_MIN_HOLD_DAYS > 0 and hold_days < KOSPI_MIN_HOLD_DAYS and not BE_STOP_FIRED_KP:
                        IsSellGo = False
